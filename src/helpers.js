...
// Calculates average times each survey takes, returns an array of objects containing surveyId, averageTime, timesTaken
const getAverages = async () => {
    ...
    surveys.forEach((survey) => {
      const timeTakenForSurvey = Math.ceil(
        (new Date(survey.endTime) - new Date(survey.startTime)) / (1000 * 60)
      );
...
// Makes a get request (count) times to endpoint to retrieve surveys.
...
// BONUS
// Returns string of statistics for a given array of survey objects.
const getStats = (surveys) => {
    let mostPopularSurvey = { timesTaken: 1 };
    let leastPopularSurvey = { timesTaken: Infinity };
    surveys.forEach((survey) => {
      if (survey.timesTaken > mostPopularSurvey.timesTaken)
        mostPopularSurvey = survey;
      if (survey.timesTaken < leastPopularSurvey.timesTaken)
        leastPopularSurvey = survey;
    });
    ...
    surveys.forEach((survey) => {
      ...
      if (survey.averageTime < shortestSurvey.averageTime)
        shortestSurvey = survey;
    });
...
// Collects data by logging results to a text file.
...
module.exports = {getAverages, getSurveys, getStats, logResults};